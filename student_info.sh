...
#select course from students full join majors using(major_id) full join majors_courses using(major_id) full join courses using(course_id) where student_id is nullgroup by course having count(course) = 1